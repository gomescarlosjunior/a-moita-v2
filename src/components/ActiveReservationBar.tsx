...
export default function ActiveReservationBar() {
  ...
  const [calPos, setCalPos] = useState<{ left: number; top: number; width: number }>({ left: 0, top: 0, width: 0 })
  const [guestPos, setGuestPos] = useState<{ left: number; top: number; width: number }>({ left: 0, top: 0, width: 280 })
  ...
  useEffect(() => {
    const updatePos = () => {
      ...
      const gWidth = 280
      const gLeft = Math.min(Math.max(8, rect.right - gWidth), Math.max(8, vw - gWidth - 8))
  ...
  useEffect(() => {
    const updateGuests = () => {
      ...
      const vw = window.innerWidth
      const gWidth = 280
      const left = Math.min(Math.max(8, rect.right - gWidth), Math.max(8, vw - gWidth - 8))
      const top = rect.bottom + 8
  ...
  return (
    <div ref={barRef} className="relative z-[200] mx-auto w-full max-w-[40.8rem]"> {/* ~15% menor que 48rem (max-w-3xl) */}
      ...
      <style jsx global>{`
        ...
        .rdp-custom { --rdp-cell-size: 28px; --rdp-accent-color: #0d2b24; --rdp-background-color: #f0f7f4; --rdp-accent-color-dark: #0a1f1a; --rdp-background-color-dark: #0d2b24; --rdp-outline: 2px solid var(--rdp-accent-color); --rdp-outline-selected: 2px solid #0d2b24; margin: 0; }
        @media (min-width: 640px) { .rdp-custom { --rdp-cell-size: 30px; } }
        @media (min-width: 768px) { .rdp-custom { --rdp-cell-size: 32px; } }
        @media (min-width: 1024px) { .rdp-custom { --rdp-cell-size: 34px; } }
        .rdp-custom .rdp-day { height: var(--rdp-cell-size); width: var(--rdp-cell-size); border-radius: 50%; }
        /* Smaller labels and numbers */
        .rdp-custom .rdp-caption_label { font-size: 1rem; }
        @media (min-width: 768px) { .rdp-custom .rdp-caption_label { font-size: 1.05rem; } }
        .rdp-custom .rdp-button { font-size: 0.85rem; }
        @media (min-width: 768px) { .rdp-custom .rdp-button { font-size: 0.9rem; } }
        .rdp-custom .rdp-head_cell { font-size: 0.75rem; }
        .rdp-custom .rdp-day_selected,
        .rdp-custom .rdp-day_selected:focus-visible,
        .rdp-custom .rdp-day_selected:hover { background-color: #0d2b24 !important; color: #fff !important; }
        .rdp-custom .rdp-day_range_start,
        .rdp-custom .rdp-day_range_end { background-color: #0d2b24 !important; color: #fff !important; }
        .rdp-custom .rdp-day_range_middle { background: rgba(13,43,36,0.1) !important; color: #0d2b24 !important; }
        .rdp-custom .rdp-nav_button { color: #0d2b24 !important; }
        .rdp-custom .rdp-nav_button:hover { background: rgba(13,43,36,0.1) !important; }
        /* Force two months side-by-side on md+ */
        @media (min-width: 768px) { .rdp-custom .rdp-months { display: grid !important; grid-template-columns: repeat(2, auto) !important; gap: 1rem; } }
      `}</style>
      {/* Screen Overlay (Portal) */}
      {mounted && (openCalendar || openGuests) && createPortal(
        <div
      ...
      <div className="flex items-center rounded-full bg-white py-1 px-3 shadow-lg ring-1 ring-black/5">
        {/* Check In */}
        <div className="relative flex-1">
          ...
          >
            <div>
              <p className="text-xs font-semibold" style={{ color: '#0d2b24' }}>Check in</p>
              <p className="text-xs text-gray-500">
                {range?.from ? format(range.from!, "dd 'de' MMM", { locale: ptBR }) : 'Adicionar datas'}
              </p>
            </div>
          </button>
        </div>
        ...
        <div className="relative flex-1">
          ...
          >
            <div>
              <p className="text-xs font-semibold" style={{ color: '#0d2b24' }}>Check out</p>
              <p className="text-xs text-gray-500">
                {range?.to ? format(range.to!, "dd 'de' MMM", { locale: ptBR }) : 'Adicionar datas'}
              </p>
            </div>
          </button>
        </div>
        ...
        <div className="relative flex-1">
          ...
          >
            <div>
              <p className="text-xs font-semibold" style={{ color: '#0d2b24' }}>Hóspedes</p>
              <p className="text-xs text-gray-500">
                {totalPeople > 0
        ...
        <button
          ...
          onMouseEnter={(e) => !isSearchDisabled && (e.currentTarget.style.backgroundColor = '#0a1f1a')}
          onMouseLeave={(e) => !isSearchDisabled && (e.currentTarget.style.backgroundColor = '#0d2b24')}
          aria-label="Reservar"
        >
          Reservar
        </button>
      ...
      {mounted && openCalendar && createPortal(
        <div
          className="fixed z-[210] rounded-xl bg-white p-2 sm:p-3 shadow-2xl ring-1 ring-black/5 max-h-[75vh] overflow-auto"
          style={{ left: calPos.left, top: calPos.top, width: calPos.width, isolation: 'isolate' }}
        >
          <DayPicker
            mode="range"
            selected={range}
          ...
          <div className="mt-3 flex justify-end">
            <button
              ...
              onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = '#0a1f1a')}
              onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = '#0d2b24')}
            >
              Fechar
            </button>
          </div>
      ...
      {mounted && openGuests && createPortal(
        <div
        ...
        >
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium" style={{ color: '#0d2b24' }}>Adultos</p>
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  onMouseEnter={(e) => !e.currentTarget.disabled && (e.currentTarget.style.backgroundColor = '#f0f9ff')}
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium" style={{ color: '#0d2b24' }}>Crianças</p>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium" style={{ color: '#0d2b24' }}>Bebês</p>
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="w-6 text-center text-sm">{guests.babies}</span>
                <button
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium" style={{ color: '#0d2b24' }}>Pets</p>
