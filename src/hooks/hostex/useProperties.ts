...
export interface UsePropertiesReturn {
  ...
  connectChannel: (propertyId: string, channelId: string, credentials: Record<string, any>) => Promise<void>
...
export function useProperties(): UsePropertiesReturn {
  ...
  const syncProperty = useCallback(async (propertyId: string) => {
    try {
      const response = await fetch('/api/hostex/properties', {
        method: 'POST',
        headers: {
  ...
  const connectChannel = useCallback(async (
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao conectar canal'
      setError(errorMessage)
      throw err
    }
  }, [refreshProperties])

  const disconnectChannel = useCallback(async (propertyId: string, channelId: string) => {
    try {
      const response = await fetch('/api/hostex/properties', {
        method: 'POST',
        headers: {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao desconectar canal'
      setError(errorMessage)
      throw err
    }
  }, [refreshProperties])
