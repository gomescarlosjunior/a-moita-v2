...
export default function IntegrationTestPage() {
    ...
    },
    {
      ...
      steps: [
        ...
        },
        {
          ...
          description: 'Carrega informações detalhadas de uma propriedade específica',
          status: 'pending'
        },
        {
          id: 'property-metrics',
    ...
    const tests = [
      async () => {
        ...
        const mockReservations = [
          { id: 'res1', guestName: 'João Silva', checkIn: '2024-01-15', status: 'confirmed' },
          { id: 'res2', guestName: 'Maria Santos', checkIn: '2024-01-20', status: 'pending' }
        ]
      ...
      async () => {
        ...
        return { 
          ...
          data: { syncedChannels: ['Airbnb', 'Booking.com'], lastSync: new Date().toISOString() }
        }
      }
    ]
    ...
    const tests = [
      async () => {
        ...
        const templates = [
          ...
          { id: 'checkin', name: 'Lembrete Check-in', trigger: 'checkin_reminder' },
          { id: 'checkout', name: 'Instruções Check-out', trigger: 'checkout_reminder' },
      ...
      async () => {
        ...
        const variables = ['{{guestName}}', '{{checkInDate}}', '{{propertyName}}', '{{totalAmount}}']
        return { 
    ...
    const tests = [
      ...
      async () => {
        ...
        return { 
          ...
          data: { connectedChannels: channels, availableChannels: activeChannels }
        }
      }
    ]
  ...
  const runTestSteps = async (suiteId: string, tests: (() => Promise<any>)[]) => {
  ...
  const getOverallStatus = () => {
    ...
    const successCount = allSteps.filter(step => step.status === 'success').length
    const errorCount = allSteps.filter(step => step.status === 'error').length
    const totalCount = allSteps.length

    if (errorCount > 0) {
      return { status: 'error', message: `${errorCount} testes falharam de ${totalCount}` }
    } else if (successCount === totalCount) {
      return { status: 'success', message: 'Integração completa validada com sucesso!' }
    } else {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Validação end-to-end: Autenticação → Propriedades → Reservas → Mensagens → Dashboard
          </p>
        </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-3">
              ...
              >
                ...
                <span>{isRunning ? 'Executando...' : 'Executar Teste Completo'}</span>
              </button>
            </div>
          </div>
        </motion.div>
